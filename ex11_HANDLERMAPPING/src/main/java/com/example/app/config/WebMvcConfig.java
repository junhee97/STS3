...
@Configuration
@EnableWebMvc
@ComponentScans({ @ComponentScan("com.example.app.controller"), @ComponentScan("com.example.app.RestController") })
...
public class WebMvcConfig implements WebMvcConfigurer {
	...
	// 2) SimpleUrlHandlerMapping : 개발자가 직접 매핑정보를 설정하는 방식 중 하나, 기본값 : 정적자원 매핑
	@Bean
	...
	// 적절한 컨트롤러 및 메소드를 찾아 매핑(@RequestMapping, @GetMapping, @PostMapping...)
	@Bean
	RequestMappingHandlerMapping requestMappingHandlerMapping() throws NoSuchMethodException, SecurityException {
		RequestMappingHandlerMapping handlerMapping = new RequestMappingHandlerMapping();

		// URL 요청 매핑정보 구성
		RequestMappingInfo mappingInfo = RequestMappingInfo.paths("/custom_03").methods(RequestMethod.GET).build();
